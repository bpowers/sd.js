...
define('draw',["require", "exports", './runtime', "./util", './xmile', "../bower_components/hammerjs/hammer", "../bower_components/Snap.svg/dist/snap.svg"], function (require, exports, runtime, util_1, xmile_1) {
    ...
    var DConnector = (function () {
        ...
        DConnector.prototype.draw = function () {
            ...
            var tx = toEnt.cx;
            var ty = toEnt.cy;
            var takeoffθ = (this.e.angle % 360) / 180 * Math.PI;
            var slopeOfTangent = Math.tan(takeoffθ);
            var perpSlopeTakeoff = -1 / slopeOfTangent;
            var rayX = 25 * Math.sin(takeoffθ);
            var rayY = 25 * Math.cos(takeoffθ);
            var rayPath = 'M' + fx + ',' + fy + 'L' + (fx + rayX) + ',' + (fy + rayY);
            var perpTakeoffθ = Math.atan(perpSlopeTakeoff);
            var prayX = 20 * Math.sin(perpTakeoffθ);
            var prayY = 20 * Math.cos(perpTakeoffθ);
            var prayPath = 'M' + fx + ',' + fy + 'L' + (fx + prayX) + ',' + (fy + prayY);
            ...
            if (fy === ty) {
                ...
                pBisectθ = Math.atan(perpBisectorSlope);
            ...
            var pbrayY = 25 * Math.sin(pBisectθ);
            var pbrayX = -25 * Math.cos(pBisectθ);
            ...
            var cx = this.e.x;
            var cy = this.e.y;
            var cr = Math.sqrt(Math.pow(fx - ourCx, 2) + Math.pow(fy - ourCy, 2));
            var circ = { r: cr, x: ourCx, y: ourCy };
            var spath = '';
            var inv = 0;
            spath += 'M' + fx + ',' + fy;
            var endθ;
            ...
            this.set = this.drawing.group(paper.path(midPath).attr({ 'stroke-width': .5, stroke: '#CDDC39', fill: 'none' }), paper.circle(midx, midy, 2).attr({ 'stroke-width': 0, fill: '#CDDC39' }), paper.circle(ourCx, ourCy, cr).attr({ 'stroke-width': .5, stroke: '#2299dd', fill: 'none' }), paper.circle(ourCx, ourCy, 2).attr({ 'stroke-width': 0, fill: '#2299dd' }), paper.circle(cx, cy, 2).attr({ 'stroke-width': 0, fill: '#c83639' }), paper.path(rayPath).attr({ 'stroke-width': .5, stroke: '#009688', 'fill': 'none' }), paper.path(prayPath).attr({ 'stroke-width': .5, stroke: '#8BC34A', 'fill': 'none' }), paper.path(pbrayPath).attr({ 'stroke-width': .5, stroke: '#FF9800', 'fill': 'none' }));
        };
