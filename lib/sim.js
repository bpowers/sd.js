...
define(['./util', './vars', './common'], function(util, vars, common) {
    ...
    */
    Sim.prototype.runTo = function(aTime) {
        // euler
        while (this.curr.time <= aTime) {
            this._model.step(this, this._ts.dt);

            if (this._stepNum % this._saveEvery === 0) {
                this._timeSeries.push(this.curr.time);
                this.series.push(this.curr);
            }
            this._stepNum++;

            this.next.time = this.curr.time + this._ts.dt;
            this.curr = this.next;
            this.next = {}
        }
    ...
    */
    Sim.prototype.runToEnd = function() {
        return this.runTo(this.time.end);
    }
    /**
    ...
    */
    Sim.prototype.getValue = function(varName) {
        return this.series[this.series.length-1][varName];
    }
    /**
    ...
    */
	...
	for (i = 0; i < this.series.length; i++) {
	    data.push(this.series[i][varName]);
	}
	result.push(data);
        return result;
    }
