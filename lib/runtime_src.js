...
Simulation.prototype.reset = function() {
    ...
    this.slab = new Float64Array(this.nVars*(nSteps+2));

    // +1 for the implicit time var
    this.curr = this.slab.subarray(0, this.nVars);
    this.next = this.slab.subarray(this.nVars, 2*this.nVars);
...
Simulation.prototype.runTo = function(endTime) {
    ...
    while (this.curr[TIME] <= endTime) {
        ...
        this.curr = this.next;
        this.next = this.slab.subarray((this.stepNum+1)*this.nVars, (this.stepNum+2)*this.nVars);
    }
};
...
Simulation.prototype.value = function(name) {
    ...
    if (off === -1)
        return;
    return this.history[Math.max(this.saveNum-1, 0)][off];
};
Simulation.prototype.series = function(name) {
    var time = new Float64Array(this.saveNum);
    var values = new Float64Array(this.saveNum);
    var off = this.lookupOffset(name);
    if (off === -1)
        return;
