define(['./util', './vars', './common'], function(util, vars, common) {
    ...
    var DStock = function (drawing, element) {
        ...
        this.label_side = element.getAttribute('label_side') || 'top';
    };
    ...
    DStock.prototype.draw = function() {
        ...
        this.set = this.drawing.group(
            ...
            label(paper, this.cx, this.cy, this.label_side, this.dName, w/2, h/2)
        );
    };
    ...
    DModule.prototype.visualize = function() {
        // TODO(bp) implement
    };
    ...
    DAux.prototype.draw = function() {
        ...
        this.set = this.drawing.group(
            ...
            label(paper, this.cx, this.cy, this.label_side, this.dName)
        );
    };
    ...
    DFlow.prototype.draw = function() {
        ...
        this.set = this.drawing.group();
        if (!this.from) {
            cloud = cloudAt(paper, floatAttr(pts[0], 'x'), floatAttr(pts[0], 'y'));
        ...
        lastPt = last(pts);
        this.set.add(arrowhead(paper, floatAttr(lastPt, 'x'), floatAttr(lastPt, 'y'), ARROWHEAD_RADIUS*2).attr({
        ...
        this.set.add(label(paper, cx, cy, this.label_side, this.dName));
    };
    ...
    */
    var Drawing = function(model, svgElementID, overrideColors, enableMousewheel) {
        common.err = null;
