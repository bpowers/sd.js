...
var DConnector = (function () {
    ...
    DConnector.prototype.drawArc = function () {
        ...
        var spath = 'M' + takeoffX + ',' + takeoffY + 'A' + circ.r + ',' + circ.r + ' 0 ' + (+sweep) + ',' + (+inv) + ' ' + tx + ',' + ty;
        var θ = 0;
        ...
        if (inv)
            θ += 180;
        if (!straightLine)
            this.set = this.drawing.group(paper.path(midPath).attr({ 'stroke-width': .5, stroke: '#CDDC39', fill: 'none' }), paper.circle(midx, midy, 2).attr({ 'stroke-width': 0, fill: '#CDDC39' }), paper.circle(circ.x, circ.y, circ.r).attr({ 'stroke-width': .5, stroke: '#2299dd', fill: 'none' }), paper.circle(circ.x, circ.y, 2).attr({ 'stroke-width': 0, fill: '#2299dd' }), paper.path(spath).attr({
                'stroke-width': STROKE / 2,
                'stroke': this.color,
                'fill': 'none',
