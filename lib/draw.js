define(['./util', './vars', './common'], function(util, vars, common) {
    ...
    DConnector.prototype.draw = function() {
        ...
        if (circ) {
            ...
            inv = endθ - startθ > 0 && endθ - startθ <= 180;
        ...
        var p = arrowhead(paper, i.x, i.y, 4).attr({
    ...
    */
    var Drawing = function(model, svgElementID) {
        ...
        $(svgElementID).mousedown(this, function(e) {
            const drawing = e.data;
            drawing.dragStart = {x:e.offsetX, y:e.offsetY};
        });

        $(svgElementID).mousemove(this, function(e) {
            const drawing = e.data;
            if (!drawing.dragStart)
                return;
            const dx = e.offsetX-drawing.dragStart.x;
            const dy = e.offsetY-drawing.dragStart.y;
        });

        $(svgElementID).mouseup(this, function(e) {
            const drawing = e.data;
