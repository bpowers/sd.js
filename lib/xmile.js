    ...
    ViewElement.Build = function (el) {
        ...
        for (var i = 0; i < el.attributes.length; i++) {
            ...
            switch (attr_8.name.toLowerCase()) {
                case 'name':
                    viewEl.name = canonicalize(attr_8.value);
                    break;
                case 'x':
                    _a = num(attr_8.value), viewEl.x = _a[0], err = _a[1];
                    if (err)
                        return [null, new Error('x: ' + err.error)];
                    break;
                case 'y':
                    _b = num(attr_8.value), viewEl.y = _b[0], err = _b[1];
                    if (err)
                        return [null, new Error('y: ' + err.error)];
                    break;
                case 'width':
                    _c = num(attr_8.value), viewEl.width = _c[0], err = _c[1];
                    if (err)
                        return [null, new Error('width: ' + err.error)];
                    break;
                case 'height':
                    _d = num(attr_8.value), viewEl.height = _d[0], err = _d[1];
                    if (err)
                        return [null, new Error('height: ' + err.error)];
                    break;
                case 'label_side':
                    viewEl.labelSide = attr_8.value.toLowerCase();
                    break;
                case 'label_angle':
                    _e = num(attr_8.value), viewEl.labelAngle = _e[0], err = _e[1];
                    if (err)
                        return [null, new Error('label_angle: ' + err.error)];
                    break;
                case 'color':
                    viewEl.color = attr_8.value.toLowerCase();
                    break;
            }
        }
        return [viewEl, err];
    ...
    View.Build = function (el) {
        ...
        for (var i = 0; i < el.attributes.length; i++) {
            ...
            switch (attr_9.name.toLowerCase()) {
                case 'type':
                    view.type = attr_9.value.toLowerCase();
                    break;
                case 'order':
                    _a = num(attr_9.value), view.order = _a[0], err = _a[1];
                    if (err)
                        return [null, new Error('order: ' + err.error)];
                    break;
                case 'width':
                    _b = num(attr_9.value), view.width = _b[0], err = _b[1];
                    if (err)
                        return [null, new Error('width: ' + err.error)];
                    break;
                case 'height':
                    _c = num(attr_9.value), view.height = _c[0], err = _c[1];
                    if (err)
                        return [null, new Error('height: ' + err.error)];
                    break;
                case 'zoom':
                    _d = num(attr_9.value), view.zoom = _d[0], err = _d[1];
                    if (err)
                        return [null, new Error('zoom: ' + err.error)];
                    break;
                case 'scroll_x':
                    _e = num(attr_9.value), view.scrollX = _e[0], err = _e[1];
                    if (err)
                        return [null, new Error('scroll_x: ' + err.error)];
                    break;
                case 'scroll_y':
                    _f = num(attr_9.value), view.scrollY = _f[0], err = _f[1];
                    if (err)
                        return [null, new Error('scroll_y: ' + err.error)];
                    break;
                case 'background':
                    view.background = attr_9.value.toLowerCase();
                    break;
                case 'page_width':
                    _g = num(attr_9.value), view.pageWidth = _g[0], err = _g[1];
                    if (err)
                        return [null, new Error('page_width: ' + err.error)];
                    break;
                case 'page_height':
                    _h = num(attr_9.value), view.pageHeight = _h[0], err = _h[1];
                    if (err)
                        return [null, new Error('page_height: ' + err.error)];
                    break;
                case 'page_sequence':
                    view.pageSequence = attr_9.value.toLowerCase();
                    break;
                case 'page_orientation':
                    view.pageOrientation = attr_9.value.toLowerCase();
                    break;
                case 'show_pages':
                    _j = bool(attr_9.value), view.showPages = _j[0], err = _j[1];
                    if (err)
                        return [null, new Error('show_pages: ' + err.error)];
                    break;
                case 'home_page':
                    _k = num(attr_9.value), view.homePage = _k[0], err = _k[1];
                    if (err)
                        return [null, new Error('home_page: ' + err.error)];
                    break;
                case 'home_view':
                    _l = bool(attr_9.value), view.homeView = _l[0], err = _l[1];
                    if (err)
    ...
    GF.Build = function (el) {
        ...
        for (var i = 0; i < el.attributes.length; i++) {
            ...
            switch (attr_10.name.toLowerCase()) {
                case 'type':
                    table.type = attr_10.value.toLowerCase();
    ...
    Scale.Build = function (el) {
        ...
        for (var i = 0; i < el.attributes.length; i++) {
            ...
            switch (attr_11.name.toLowerCase()) {
                case 'min':
                    _a = num(attr_11.value), scale.min = _a[0], err = _a[1];
                    if (err)
                        return [null, new Error('bad min: ' + attr_11.value)];
                    break;
                case 'max':
                    _b = num(attr_11.value), scale.max = _b[0], err = _b[1];
                    if (err)
                        return [null, new Error('bad max: ' + attr_11.value)];
                    break;
            }
        }
    ...
    Connection.Build = function (el) {
        ...
        for (var i = 0; i < el.attributes.length; i++) {
            ...
            switch (attr_12.name.toLowerCase()) {
                case 'to':
                    conn.to = canonicalize(attr_12.value);
                    break;
                case 'from':
                    conn.from = canonicalize(attr_12.value);
                    break;
            }
        }
