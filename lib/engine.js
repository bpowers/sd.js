...
ERR_VERSION  = engine.ERR_VERSION  = "bad xml or unknown smile version";
ERR_BAD_TIME = engine.ERR_BAD_TIME = "bad time (control) data";
engine.err = null;
...
*/
engine.sort = function quicksort(array, l, r) {
    if (l === undefined) {
	l = 0;
	r = array.length - 1;
    }
...
Model.prototype._parseVars = function(varList) {
    ...
    varList.map(function() {
	    ...
	    v = new Variable(name, type, eqn, inflows, outflows);
            that.initials.push(v);
            that.stocks.push(v);
        } else {
	    v = new Variable(name, type, eqn);
            that.flows.push(v);
        }
    });
