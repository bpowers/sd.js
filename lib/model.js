...
define(['./util', './vars', './common'], function(util, vars, common) {
    ...
    */
    const Model = function Model(xmlString) {
        ...
        model.ctrl = parseControlInfo(xmile.children('sim_specs'));
        if (!model.ctrl) {
            this.valid = false;
            return;
        }
        ...
        model.vars = this._parseVars(xmile.find('model[name=""] variables').children());
    ...
    */
    Model.prototype._parseVars = function(varList) {
        ...
        varList.map(function(m) { return function() {
            const jthis = $(this);
            const name = jthis.attr('name').trim().toLowerCase();
            const type = jthis.get(0).tagName.toLowerCase();
            const eqn  = jthis.children('eqn').text().trim();
            const gf   = jthis.children('gf');
            var v;
            ...
            if (type === 'stock') {
                const inflows  = textList(jthis.children('inflow'));
                const outflows = textList(jthis.children('outflow'));
