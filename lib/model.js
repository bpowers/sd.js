...
define(['./util', './vars', './common', './draw', './sim'], function(util, vars, common, draw, sim) {
    ...
    const requiredSpecs = ['start', 'stop', 'dt', 'savestep'];
    const qs = util.qs;
    ...
    */
    const Model = function Model(xmlString) {
        ...
        const xmile = (new DOMParser()).parseFromString(xmlString, 'application/xml');
        ...
        this.timespec = parseControlInfo(qs(xmile, 'xmile>sim_specs'));
        if (!this.timespec) {
            this.valid = false;
            return;
        }
    ...
    */
    const parseControlInfo = function(specs) {
        var error;
    ...
    */
    ...
    const textList = function(nodeList) {
        var result = [];
        var i;
    ...
    */
    Model.prototype._parseVars = function(xvars) {
        ...
        var i, xv, name, type, eqn, gf, v, inflows, outflows;
        for (i=0; i < xvars.length; i++) {
            ...
            eqn = xv.getElementsByTagName('eqn')[0].textContent.trim();
